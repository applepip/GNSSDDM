...
void snWriteResultFile::write(std::string path, double data) {
	...
	DataSet dataset = f->createDataSet(path.c_str(), PredType::NATIVE_DOUBLE,
			dataspace);
...
void snWriteResultFile::write(std::string path, double *data,
	...
	DataSet dset3 = f->createDataSet(path.c_str(), PredType::NATIVE_DOUBLE,
			dspace3);
	dset3.write(data, PredType::NATIVE_DOUBLE);
}

void snWriteResultFile::write(std::string path, std::vector<double> &data,
		unsigned long num) {
	hsize_t dimsf3[1]; // dataset dimensions
	dimsf3[0] = num;
	DataSpace dspace3(1, dimsf3);
...
void snWriteResultFile::write(std::string path,
		std::vector<std::complex<double> > &data, unsigned long num) {
	...
	cp_cmplx.insertMember("real", HOFFSET(hdf5_complex, real),
			PredType::NATIVE_DOUBLE);
	cp_cmplx.insertMember("imag", HOFFSET(hdf5_complex, imag),
			PredType::NATIVE_DOUBLE);
...
void snWriteResultFile::write(std::string path, std::complex<double> *data,
		unsigned long num) {
...
void snWriteResultFile::write_vec(std::string path, itpp::vec data) {
	write(path, &data[0], data.length());
	f->flush(H5F_SCOPE_GLOBAL);
}

void snWriteResultFile::write_cvec(std::string path, itpp::cvec data) {
	write(path, &data[0], data.length());
	f->flush(H5F_SCOPE_GLOBAL);
}

DataSet* snWriteResultFile::initialize_dataset(std::string path,
		unsigned long len) {

	double fillvalue = 0; /* Fill value for the dataset */
	DSetCreatPropList plist;
	plist.setFillValue(PredType::NATIVE_DOUBLE, &fillvalue);

	hsize_t fdim[] = { len }; // dim sizes of ds (on disk)
	DataSpace fspace(1, fdim); // rank = 1

	DataSet* dataset = new DataSet(
			f->createDataSet(path.c_str(), PredType::NATIVE_DOUBLE, fspace,
					plist));

	return dataset;
}

void snWriteResultFile::write_to_dataset(DataSet* dataset, unsigned long index,
		double data) {
...
//	hsize_t block = { 1 };
	...
	DataSpace mspace(1, fdim); // rank = 1
	dataset->write(&data, PredType::NATIVE_DOUBLE, mspace, fspace);
}
