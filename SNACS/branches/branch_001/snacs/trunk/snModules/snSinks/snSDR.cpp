 ...
 */
snSDR::snSDR(SNSignal pSNSignal, ConcBuf3* _circbuf) :
	snSerialModule(pSNSignal, _circbuf) {
}

void snSDR::calculate_loop_coefficients(double &tau1, double &tau2, double LBW,
...
bool snSDR::Aquisition(itpp::vec aq_signal, signed long long samplesPerChip,
	...
	Samples_Type samples_per_search_length = search_length * smpl_freq;
	...
	itpp::cvec longSignal_C(samples_per_search_length); // buffer for 11ms of signal
	...
	for (Samples_Type i = 0; i < samplesPerCode; i++)
		PhasePoints.at(i) = static_cast<double> (i) * 2.0 * M_PI * (1.0 / smpl_freq);

	// FFT code block
	for (Samples_Type i = 0; i < samplesPerCode; i++) {
		CAblkTD[i] = aq_signal[i];
	}
	...
	for (size_t o = 0; o < AqFreqs.size(); o++) {
		...
		snLog("snSDRGPS: Processing frequency: " + QString::number(AqFreqs.at(o)), snModule::debugv);
		// mix down signal
		for (Samples_Type i = 0; i < samplesPerCode; i++) {
		...
		if (SDR.switch_plot_3d) {
			...
			for (int i = 0; i < SDR.aq_plot_code_phases; i++) {
				...
				curve_aq->update_XY(i, o, sqrt(vMax) / samplesPerCode);
			}
			snw->releasePlotFlag();
		}
	...
	for (size_t s = 0; s < AqResults.size(); s++) { // for all frequencies
		...
		snLog("CodePhase for freq " + QString::number(AqFreqs.at(s)) + ": " + QString::number(
				CodePhase2) + ", value: " + QString::number(*std::max_element(
				AqResults.at(s).begin(), AqResults.at(s).end())), snModule::debugv);
	}
	...
	snLog("snSDRGPS: Aquisition metric: " + QString::number(aq_metric), snModule::debug);
	...
	if (aq_metric > SDR.AqThreshold) {
		if (0) {
			...
			double vRefinedFreq = iRefinedFreq * (smpl_freq / (double) CarrN);
			snLog("snSDR: iRefinedFreq: " + QString::number(iRefinedFreq) + ", vRefinedFreq: "
					+ QString::number(vRefinedFreq), snModule::debug);
		...
		snLog("snSDRGPS: Aquisition successfull. Maximum at freq: " + QString::number(AqFreq)
				+ ", code = " + QString::number(AqCodePhase), snModule::debug);
...
/* TODO: convert to ipp functions:
bool snSDR::Aquisition(itpp::vec aq_signal, signed long long samplesPerChip,
		double search_length, double &AqFreq, Samples_Type &AqCodePhase,
		snWidget *snw, snCurve3D *curve_aq) {

	itpp::cvec SigInTD1; ///< array for Fourier transform of signal input , time domain
	itpp::cvec SigInFD1; ///< array for  Fourier transform of signal input, frequency domain
	itpp::cvec SigInTD2; ///< array for Fourier transform of signal input , time domain
	itpp::cvec SigInFD2; ///< array for  Fourier transform of signal input, frequency domain
	itpp::cvec CAblkTD; ///< array for Fourier transform of code signal , time domain
	itpp::cvec CAblkFD; ///< array for Fourier transform of code signal, frequency domain
	itpp::cvec AqResFD1; ///< array for Fourier transform of aq result, frequency domain
	itpp::cvec AqResTD1; ///< array for Fourier transform of aq result, time domain

	itpp::cvec AqResFD2; ///< array for Fourier transform of aq result, frequency domain
	itpp::cvec AqResTD2; ///< array for Fourier transform of aq result, time domain

	Samples_Type samplesPerCode = aq_signal.size();
	Samples_Type samples_per_search_length = search_length * smpl_freq;

	itpp::cvec longSignal_C(samples_per_search_length); // buffer for 11ms of signal
	itpp::vec longSignal(samples_per_search_length); // buffer for 11ms of signal
	itpp::vec Sig0DC(samples_per_search_length); // holds zero DC signal

	itpp::vec signal1(samplesPerCode); // buffer1 for 1ms of signal
	itpp::vec signal2(samplesPerCode); // buffer2 for 1ms of signal

	// Aquisition //
	snLog("snSDRGPS: Starting aquisition.", 1);
	//for (Samples_Type i = 0; i < samples_per_search_length; i++)
	longSignal_C = get_next_samples(samples_per_search_length);
	for (int i = 0; i < longSignal_C.size(); i++)
		longSignal[i] = longSignal_C[i].real();
	...
	for (Samples_Type i = samplesPerCode; i < 2 * samplesPerCode; i++) {
		signal2[i - samplesPerCode] = longSignal[i];
	}

	double Sig11msMean = 0.0;
	for (Samples_Type i = 0; i < samples_per_search_length; i++)
		Sig11msMean += longSignal[i];

	Sig11msMean /= samples_per_search_length;
	for (Samples_Type i = 0; i < samples_per_search_length; i++)
		Sig0DC[i] = longSignal[i] - Sig11msMean;

	// create phase points with input signal:
	itpp::vec PhasePoints(samplesPerCode); ///< phase points that go into sin() and cos() are pre-calculated

	for (Samples_Type i = 0; i < samplesPerCode; i++)
		PhasePoints[i] = static_cast<double> (i) * 2.0 * M_PI / smpl_freq;
	...
	//fftw_execute(FFTCode); // F{CAblkTD} = CAblkFD
	CAblkFD = itpp::fft(CAblkTD);
	// conjugate result
	CAblkFD = itpp::conj(CAblkFD);

	itpp::vec
			AqFreqs(static_cast<int> (floor(SDR.AqFreqBand / SDR.AqFreqStep)));

	for (size_t i = 0; i < AqFreqs.size(); i++) {
		AqFreqs[i] = intermed_freq - SDR.AqFreqBand / 2.0 + i * SDR.AqFreqStep;
	}

	std::vector<itpp::vec> AqResults(AqFreqs.size()); ///< memory for aq results, vector will have size: acFreqSteps x acSmpls
	for (size_t i = 0; i < AqFreqs.size(); i++)
		AqResults[i] = itpp::vec(samplesPerCode);

	itpp::vec AqRes1(samplesPerCode);
	itpp::vec AqRes2(samplesPerCode);

	// put input data into input signal to FFT
	for (size_t o = 0; o < AqFreqs.size(); o++) {

		snLog("snSDRGPS: Processing frequency: " + QString::number(
				AqFreqs[o]), 4);
		// mix down signal
		for (Samples_Type i = 0; i < samplesPerCode; i++) {
			SigInTD1[i].real() = sin(PhasePoints[i] * AqFreqs[o]) * signal1[i];
			SigInTD1[i].imag() = cos(PhasePoints[i] * AqFreqs[o]) * signal1[i];
			SigInTD2[i].real() = sin(PhasePoints[i] * AqFreqs[o]) * signal2[i];
			SigInTD2[i].imag() = cos(PhasePoints[i] * AqFreqs[o]) * signal2[i];
		}

		//		fftw_execute(FFTSigIn1); // F{SigInTD1} = SigInFD1
		SigInFD1 = itpp::fft(SigInTD1);
		//fftw_execute(FFTSigIn2); // F{SigInTD2} = SigInFD2
		SigInFD2 = itpp::fft(SigInTD2);
		...
		// inverse fft to get aq result
		//fftw_execute(FFTAqRes1); // F^(-1){AqResFD1} = AqResTD1
		AqResTD1 = itpp::ifft(AqResFD1);
		//fftw_execute(FFTAqRes2); // F^(-1){AqResFD2} = AqResTD2
		AqResTD2 = itpp::ifft(AqResFD2);
		...
		for (Samples_Type i = 0; i < samplesPerCode; i++) {
			AqResTD1[i] /= samplesPerCode;
			AqResTD2[i] /= samplesPerCode;
			AqRes1[i] = itpp::norm(AqResTD1[i]);
			AqRes2[i] = itpp::norm(AqResTD2[i]);
		}

		double maxAQRes1 = itpp::max(AqRes);
		double maxAQRes2 = itpp::max(AqRes);

		if (maxAQRes1 > maxAQRes2) {
			for (Samples_Type i = 0; i < samplesPerCode; i++)
				AqResults[o][i] = AqRes1[i];
		} else {
			for (Samples_Type i = 0; i < samplesPerCode; i++)
				AqResults[o][i] = AqRes2[i];
		}

		if (SDR.switch_plot_3d) {
			snw->pushData();
			int fac = static_cast<int> (floor(samplesPerCode
					/ SDR.aq_plot_code_phases));
			for (int i = 0; i < SDR.aq_plot_code_phases; i++) {
				double vMax = 0.0;
				// show maximal value in aq_plot_code_phases interval
				for (int l = i * fac; l < (i + 1) * fac; l++) {
					double tmp = AqResults[o][l];
					if (tmp > vMax)
						vMax = tmp;
				}
	...
	// find maximum:
	itpp::vec vMaximums(AqResults.size());
	for (size_t o = 0; o < AqResults.size(); o++) { // for all frequencies
		vMaximums[o] = itpp::max(AqResults[o]);
	}

	double PeakSize = itpp::max(vMaximums);

	int iMaxFreq = itpp::max_index(vMaximums);

	for (size_t s = 0; s < AqResults.size(); s++) { // for all frequencies
		Samples_Type CodePhase2 = itpp::max_index(AqResults[s]);

		snLog("CodePhase for freq " + QString::number(AqFreqs[s]) + ": "
				+ QString::number(CodePhase2) + ", value: " + QString::number(
				itpp::max_index(AqResults)), 6);
	}

	Samples_Type CodePhase = itpp::max_index(AqResults[?]);

	// Find one chip wide C/A code phase exclude range around the peak

	itpp::vec aq_fbin = AqResults[iMaxFreq];
	//	set 2 chip wide bin range around the found max peak in aq_fbin to zero:
	for (signed long long i = -samplesPerChip; i < samplesPerChip; i++) {
		signed long long idx = CodePhase + i;
	...
	double secPeakSize = itpp::max(aq_fbin.begin);

	bool AqSuccess;

	double aq_metric = PeakSize / secPeakSize;

	snLog("snSDRGPS: Aquisition metric: " + QString::number(aq_metric), 1);

	if (aq_metric > SDR.AqThreshold) {
		if (0) {
			// freq refinement
			//			snGPS1.OneCodeBlkSampled(CAblkTD, smpl_freq, SDR.AqLen, 10);
			double longCA[10 * samplesPerCode]; // 10ms of CA code
			for (int o = 0; o < 10; o++) {
				for (Samples_Type i = 0; i < samplesPerCode; i++)
					longCA[o * samplesPerCode + i] = CAblkTD[i].real();
			...
			CarrTD = new std::complex<double>[CarrN];
			CarrFD = new std::complex<double>[CarrN];
			FFTCarr = fftw_plan_dft_1d(samplesPerCode,
					reinterpret_cast<fftw_complex*> (CarrTD),
					reinterpret_cast<fftw_complex*> (CarrFD), FFTW_FORWARD,
					FFTW_ESTIMATE);
			...
			itpp::vec FineFreq(CarrN);
			for (int i = 0; i < CarrN; i++) {
				FineFreq[i] = abs(CarrFD[i]);
			}

			//double iRefinedFreq = std::max_element ( FineFreq.begin() + 5, FineFreq.end() - ( CarrN/2-5 ) ) - FineFreq.begin();
			double iRefinedFreq = std::max_element(FineFreq.begin()
					+ FineFreq.size() / 2, FineFreq.end()) - FineFreq.begin();
			double vRefinedFreq = iRefinedFreq * (smpl_freq / (double) CarrN);
			snLog("snSDR: iRefinedFreq: " + QString::number(iRefinedFreq)
					+ ", vRefinedFreq: " + QString::number(vRefinedFreq), 4);
		...
		AqFreq = AqFreqs[iMaxFreq];
		AqCodePhase = CodePhase;

		snLog("snSDRGPS: Aquisition successfull. Maximum at freq: "
				+ QString::number(AqFreq) + ", code = " + QString::number(
				AqCodePhase), 4);
...
double snSDR::discriminator_dot_product_normalized(double I_E, double Q_E,
		double I_P, double Q_P, double I_L, double Q_L) {
	double code_error = (I_P * (I_E - I_L) + Q_P * (Q_E - Q_L)) / (I_P * I_P
			+ Q_P * Q_P);
	return code_error;
}

double snSDR::discriminator_early_minus_late_power_normalized(double I_E,
		double Q_E, double I_L, double Q_L) {
