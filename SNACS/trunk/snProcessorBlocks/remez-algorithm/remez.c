...
/*
 * File originally by Jake Janovetz, modified by Frank Schubert
 */
...
void CreateDenseGrid(int r, int numtaps, int numband, double bands[], double des[], double weight[],
		int *gridsize, double Grid[], double D[], double W[], int symmetry) {
	int i, j, k, band;
...
void CalcError(int r, double ad[], double x[], double y[], int gridsize, double Grid[], double D[],
		double W[], double E[]) {
	int i;
	double A;
...
void Search(int r, int Ext[], int gridsize, double E[]) {
	...
	if (RDEBUG)
		printf("c1. r: %i\n", r);
	int foundExt_maxindex = 4 * r;
	int foundExt[foundExt_maxindex];
	k = 0;
	...
	for (i = 1; i < gridsize - 1; i++) {
		if (((E[i] >= E[i - 1]) && (E[i] > E[i + 1]) && (E[i] > 0.0)) || ((E[i] <= E[i - 1]) && (E[i] < E[i
				+ 1]) && (E[i] < 0.0))) {
			if ((k >= foundExt_maxindex)) {
				printf("s0 error. r: %i, k: %i. accessing foundExt[k]. \n", r, k);
				exit(1);
			}
			foundExt[k++] = i;
		}
	...
	while (extra > 0) {
		...
		alt = 1;
		for (j = 1; j < k; j++) {
			if ((j >= foundExt_maxindex) || (l >= foundExt_maxindex)) {
				printf("s1 error\n");
				exit(1);
			}
		...
		{
			if ((j >= foundExt_maxindex) || (j + 1 >= foundExt_maxindex)) {
				printf("s3 error\n");
				exit(1);
			}
...
int remez(double h[], int numtaps, int numband, double bands[], double des[], double weight[], int type) {
	...
	for (iter = 0; iter < MAXITERATIONS; iter++) {
		...
		if (RDEBUG) printf("calling Search, r: %i\n", r);
		Search(r, Ext, gridsize, E);
		if (isDone(r, Ext, E))
			break;
	}
	if (iter == MAXITERATIONS) {
		printf("remez.c: reached maximum iteration count.\nResults may be bad.\n");
		return 1;
	}
