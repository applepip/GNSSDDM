...
pseudoranges = hdf5read(file, '/snSDRGPS/output/pr');

epochs = length(pseudoranges);
sim_length = epochs * corr_length;
if (plotting.length ~= 0)
    if sim_length < plotting.length
        error('provided parameter len: %f is too long for snacs result file.', len);
    end
    if sim_length > plotting.length
...
hold on; grid on;
plot(dt, 10*log10(cn0), 'b-','LineWidth', plotting.line_width);
legend('C/N_0');
xlabel('Time [s]');
ylabel('dB');
xlim([ 0, sim_length ]);
